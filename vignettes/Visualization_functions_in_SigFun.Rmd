...
Currently, visualization functions in SigFun only support the result object of
the clusterProfiler package. [See clusterProfiler for more detail.](https://www.bioconductor.org/packages/release/bioc/html/clusterProfiler.html) 
Here is just one example for your reference.
```{r Method_2, warning=FALSE, eval=FALSE}
# Method 2:  Use your own GSEA results by modifying the cor.df and gseaResult
# Run GSEA using clusterProfiler
# Just replace these variables with your own analysis results
geneList <- SE_data.res@metadata$cor.df["cor"] %>% unlist()
names(geneList) <- SE_data.res@metadata$cor.df$gene
...
S4Vectors::metadata(SE_data.res) <- list(
    gseaResult=clusterProfiler,
    cor.df=S4Vectors::metadata(SE_data.res)$cor.df)
...
```{r transform, warning=FALSE, eval=FALSE}
...
```{r plot_heat_signaling, warning=FALSE, eval=FALSE}
# Use plot_heat to generate a integrative heatmap
...
library(ggplot2)
res.barplot <- barPlot(SE_data.fgsea=SE_data.res,
        topN=10,
        breaklineN=30,
        alpha="pvalue")
...
res.chord <- chordPlot(SE_data.fgsea=SE_data.res,
        fontSize=0.5,
        breaklineN=30,
        showCategory=2)
...
res.cne <- cnetPlot(SE_data.fgsea=SE_data.res,
    showCategory=5, 
    node_label='category',
    fontSize=1.2, 
    color=TRUE, 
...
res.dot <- dotPlot(SE_data.fgsea=SE_data.res,
        showCategory=10,
        breaklineN=20,
        color='pvalue')
...
res.ema <- emapPlot(SE_data.fgsea=SE_data.res,
        showCategory=10)
...
gseaPlot(SE_data.fgsea=SE_data.res,
        showCategory=5)
...
res.heat <- heatPlot(SE_data.fgsea=SE_data.res,
        breaklineN=30, 
        showCategory=2, 
        dotSize='pval', 
        dotColor='cor')
...
res.lollipop <- lollipopPlot(SE_data.fgsea=SE_data.res,
        showCategory=10,
        breaklineN=30)
...
res.ridge <- ridgePlot(SE_data.fgsea=SE_data.res,
        showCategory=10,
        breaklineN=30)
...
res.tree <- treePlot(SE_data.fgsea=SE_data.res,
         showCategory=10, 
         color='pvalue')
...
res.upsetPlot <- upsetPlot(SE_data.fgsea=SE_data.res,
        showCategory=5,
        breaklineN=30,
        type="box")
