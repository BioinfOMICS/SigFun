...
# Robust mocking helper for .plotEnrichmentData
with_mocked_enrichment_data <- function(code) {
    ...
    on.exit({
        ...
        } else if (exists(".plotEnrichmentData", envir = ns, inherits = FALSE)) {
...
test_that(".plotEnrichment basic plot builds without error and returns ggplot", {
...
test_that(".plotEnrichment positive color produces '#D25C43' for tick segments", {
    with_mocked_enrichment_data({
        p <- .plotEnrichment(
            pathway = c("g"),
            stats = c(g = 1, h = 0),
        ...
        expect_true(all(tolower(seg_data$colour) == "#d25c43"))
        expect_equal(unique(seg_data$linewidth), 0.3)
    })
})

test_that(".plotEnrichment negative color produces '#5979A3' for tick segments", {
    with_mocked_enrichment_data({
        p <- .plotEnrichment(
            pathway = c("g"),
            stats = c(g = 1, h = 0),
        ...
        built <- ggplot2::ggplot_build(p)
        seg_data <- built$data[[1]]
        expect_true(all(tolower(seg_data$colour) == "#5979a3"))
    })
})

test_that(".plotEnrichment ticksSize modifies segment linewidth", {
    with_mocked_enrichment_data({
