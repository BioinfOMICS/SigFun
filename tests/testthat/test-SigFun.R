...
test_that("SigFun processes data correctly and creates expected outputs", {
...
test_that("SigFun handles invalid inputs appropriately", {
  temp_dir <- tempdir()
  data(demo)
  SE_data@metadata$cor.df <- NULL
  SE_data@metadata$fgseaRes <- NULL
...
test_that("SigFun handles parameter variations correctly", {
  temp_dir <- tempdir()
  data(demo)
  SE_data@metadata$cor.df <- NULL
  SE_data@metadata$fgseaRes <- NULL
  ...
  expect_warning(result_pearson <- SigFun(
    SE_data = SE_data,
    ranking.method = "stat",
    cor.method = "pearson",
  ...
  expect_warning(result_spearman <- SigFun(
    SE_data = SE_data,
    ranking.method = "stat",
    cor.method = "spearman",
    pathways.all = mock_pathways,
